...
def CA_call(*, resp, year, **_):
    """
    Reads California GHG inventory data from the provided Excel file.
    Handles both web responses (requests.get) and locally loaded files.
    """
    # Check if 'resp' is a web response object (from requests.get)
    if hasattr(resp, "content"):  
        df = pd.read_excel(BytesIO(resp.content), sheet_name="Included emissions", skiprows=1,
                           keep_default_na=False, na_values=[''])  
    else:
        df = pd.read_excel(resp, sheet_name="Included emissions", skiprows=1,
                           keep_default_na=False, na_values=['']) 
...
def CA_customized_parse(df_list, source, year, config, **_):
    """
    Parses and processes the California GHG inventory data to align with FLOWSA.
    """
    df = pd.concat(df_list, ignore_index=True)

    # Filter out import-based electricity emissions
    ...
    # Clean FlowAmount only if needed
    if df['FlowAmount'].dtype == object or pd.to_numeric(df['FlowAmount'], errors='coerce').isna().any():
        df['FlowAmount'] = pd.to_numeric(df['FlowAmount'], errors='coerce')
        df.dropna(subset=['FlowAmount'], inplace=True)

    # Metadata and formatting
    df['Description'] = 'California GHGI'
    df['Class'] = 'Chemicals'
    ...
    # Add FIPS (2015 system, needed by flowsa)
    df['State'] = 'CA'
...
if __name__ == '__main__':
    ...
    for y in range(2019, 2020):
        flowsa.generateflowbyactivity.main(source='StateGHGI_CA', year=y)
        fba = flowsa.flowbyactivity.getFlowByActivity('StateGHGI_CA', y)
